...
uint32_t func_debug[500]; 											// array used to store realtime data 	

int main(void){
  				
	uint32_t 	count = 0;														// my loop control variable initialized to 0	
	...
	while(count < 500){															// run program for fixed number of steps/samples
		GPIO_PORTN_DATA_R ^= 0b00000001;							// toggle LED for visualization of process
		SysTick_Wait(120000);			         						// 1ms delay
