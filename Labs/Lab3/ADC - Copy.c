...
uint32_t func_debug;  														// array used to store realtime data 	
double freq; 											

int main(void){
  				
	uint32_t 	count = 0, prevADC = 0;														// my loop control variable initialized to 0	
	...
	while(1){															// run program for fixed number of steps/samples
		SysTick_Wait(12000);			         						// 100us delay
		func_debug = ADC0_InSeq3();					// Acquire sample
		if(func_debug < 2047 && prevADC >= 2047) { // Calculate frequency every period
			freq = 1.0/(count / 10000.0);
			count = 0;
		}
		count++;
		prevADC = func_debug; // Set previous value to this one
	}
