...
// Daniel Valvano
// May 14, 2014
// ST7735_DrawPixel bug fixed 9/2/2014
...
// SCK (pin 8) connected to PA2 (SSI0Clk) 						--> PD3 (SSI2Clk)
// MOSI (pin 7) connected to PA4 (SSI0XDAT0/SSI0Tx)  	--> PD1	(SSI2XDAT0/SSI2Tx)
// TFT_CS (pin 6) connected to PA3 (SSI0Fss)  				--> PD2	(SSI2Fss) 
// CARD_CS (pin 5) unconnected
// Data/Command (pin 4) connected to PA6 (GPIO) 			--> PD6
// RESET (pin 3) connected to PA7 (GPIO)  						--> PD7
...
#define TFT_CS                  (*((volatile uint32_t *)0x4005B010))
#define TFT_CS_LOW              0           // CS normally controlled by hardware
#define TFT_CS_HIGH             0x04
#define DC                      (*((volatile uint32_t *)0x4005B100))
#define DC_COMMAND              0
#define DC_DATA                 0x40
#define RESET                   (*((volatile uint32_t *)0x4005B200))
#define RESET_LOW               0
...
// Inputs: None
// Outputs: None
// Notes: This assumes 120 MHz system clock.
...
void static commonInit(const unsigned char *cmdList) {
  ...
  GPIO_PORTD_DIR_R |= 0xC4;             // make PD2,6,7 out
  GPIO_PORTD_AFSEL_R &= ~0xC4;          // disable alt funct on PD2,6,7
  GPIO_PORTD_DEN_R |= 0xC4;             // enable digital I/O on PD2,6,7
                                        // configure PD2,6,7 as GPIO
  GPIO_PORTD_PCTL_R = (GPIO_PORTD_PCTL_R&0x00FFF0FF)+0x00000000;
  GPIO_PORTD_AMSEL_R &= ~0xC4;          // disable analog functionality on PD2,6,7
  TFT_CS = TFT_CS_LOW;
  RESET = RESET_HIGH;
  Delay1ms(500);
  RESET = RESET_LOW;
  ...
  // initialize SSI0
  GPIO_PORTD_AFSEL_R |= 0x0E;           // enable alt funct on PA2,3,4 --> PD1,2,3
  GPIO_PORTD_DEN_R |= 0x0E;             // enable digital I/O on PA2,3,4 --> PD1,2,3
                                        // configure PA2,3,4 as SSI  --> PD1,2,3
  GPIO_PORTD_PCTL_R = (GPIO_PORTD_PCTL_R&0xFFFF000F)+0x0000FFF0;
  GPIO_PORTD_AMSEL_R &= ~0x0E;          // disable analog functionality on PA2,3,4 --> PD1,2,3
